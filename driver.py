...
class Bot:
    ...
    def prepare_message(self, row):
        """
        Prepares the message, replacing placeholders like %NAME%, %EMPLOYEE_ID%, %DATE% based on CSV row.
        """
        message = self._message
        
        # Assuming fixed column order: 0=name, 1=number (ignored here), 2=employee_id, 3=date
        # Add more as needed, e.g., if len(row) > 4, row[4] for next placeholder
        name = row[0] if len(row) > 0 else ""
        employee_id = row[2] if len(row) > 2 else ""
        date = row[3] if len(row) > 3 else ""
        
        if self._options[0]:  # If include_placeholders is True, replace name/employee_id/etc.
            if name:
                message = message.replace("%NAME%", name)
            if employee_id:
                message = message.replace("%EMPLOYEE_ID%", employee_id)
            # Add more placeholders here, e.g., message = message.replace("%OTHER%", row[4])
        
        # Always replace %DATE% if available (or remove this if you want it toggled too)
        if date:
            message = message.replace("%DATE%", date)
        
        # If a placeholder wasn't replaced (e.g., missing in CSV), it stays as-is
        return message
        ...
        try:
            with open(self._csv_numbers, mode="r") as file:
                ...
                for row in csv_reader:
                    if len(row) < 2:
                        continue  # Skip invalid rows with fewer than 2 columns
                    name = row[0]
                    number = row[1]
